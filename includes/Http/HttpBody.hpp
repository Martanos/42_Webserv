...
private:
	...
	// Parsing paths
	BodyState _parseChunkedBody(std::vector<char> &buffer, const Location &location, HttpResponse &response);
	BodyState _parseContentLengthBody(std::vector<char> &buffer, const Location &location, HttpResponse &response);
...
public:
	...
	void parseBuffer(std::vector<char> &buffer, const Location &location, HttpResponse &response);
	...
	const FileManager &getTempFileManager() const;
