...
private:
	...
	bool _serveFile(const std::string &filePath, HttpResponse &response, const Location &location);
	bool _serveDirectory(const HttpRequest &request, HttpResponse &response, const Location &location);
	bool _serveSymlink(const std::string &linkPath, HttpResponse &response, const Location &location);
