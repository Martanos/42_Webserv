...
static inline bool parseIPv4(const std::string &ipStr, uint32_t &result)
{
	...
	// Split by dots
	std::vector<std::string> octets = StrUtils::splitString(ipStr, '.');
	if (octets.size() != 4)
	{
		return false;
	}
...
static inline bool parseIPv6(const std::string &ipStr, struct in6_addr &result)
{
	// Handle special cases
	if (ipStr.empty())
		return false;
	...
	// etc.
	std::vector<std::string> parts = StrUtils::splitString(ipStr, ':');
...
static inline std::string ipv4ToString(uint32_t addr)
{
...
static inline std::string ipv6ToString(const struct in6_addr &addr)
{
	std::string result;
...
static inline bool looksLikeIPv4(const std::string &str)
{
	if (str.empty())
		return false;
...
static inline bool looksLikeIPv6(const std::string &str)
{
	if (str.empty())
		return false;
