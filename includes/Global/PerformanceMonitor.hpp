...
public:
	...
	struct Metrics
	{
		...
		Metrics()
			: totalRequestTime(0), averageRequestTime(0), minRequestTime(0), maxRequestTime(0), totalCGITime(0),
			  averageCGITime(0), totalFileReadTime(0), averageFileReadTime(0), totalConnections(0),
			  activeConnections(0), totalRequests(0), successfulRequests(0), failedRequests(0), bytesTransferred(0),
			  peakMemoryUsage(0), currentMemoryUsage(0), totalMemoryAllocated(0), fileDescriptorsUsed(0),
			  maxFileDescriptors(0), cpuUsagePercent(0), epollEventsProcessed(0), timeoutsOccurred(0), errorsOccurred(0)
		{
		}
	};
...
public:
	...
	// Metrics access
	const Metrics &getCurrentMetrics() const
	{
		return _currentMetrics;
	}
	const Metrics &getSessionMetrics() const
	{
	...
	void setPerformanceThresholds(double maxRequestTime = 1000.0, double maxCGITime = 5000.0,
...
#define PERF_START_TIMER(name)                                                                                         \
	PerformanceMonitor::Timer *perf_timer_##name = PerformanceMonitor::getInstance().createTimer(#name);               \
	perf_timer_##name->start()
#define PERF_STOP_TIMER(name)                                                                                          \
	perf_timer_##name->stop();                                                                                         \
	PerformanceMonitor::getInstance().recordRequestTime(perf_timer_##name->getElapsedTime());                          \
	PerformanceMonitor::getInstance().destroyTimer(perf_timer_##name)
