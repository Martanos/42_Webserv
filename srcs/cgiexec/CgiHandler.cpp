...
CgiHandler::CgiHandler(const CgiHandler &other) : _timeout(other._timeout), _cgiEnv(other._cgiEnv), _executor(other._timeout), _response(other._response)
{
}
...
CgiHandler::ExecutionResult CgiHandler::setupEnvironment(const HttpRequest &request,
                                                         const Server *server,
                                                         const Location *location,
                                                         ...
                                                         std::string &error)
{
    ...
    // Execute the script
    CgiExecutor::ExecutionResult execResult = _executor.execute(
        scriptPath,
        interpreter,
        envArray,
        request.getBody(),
...
{
    std::string logMessage = "CGI execution: " + request.getMethod() + " " + request.getUri() + 
                            " -> " + scriptPath + " (";
