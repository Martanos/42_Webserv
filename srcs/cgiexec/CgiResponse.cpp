...
CgiResponse::CgiResponse()
    : _statusCode(200), _statusMessage("OK"), _isParsed(false), _isNPH(false) {}
...
CgiResponse::CgiResponse(const CgiResponse &other)
    : _headers(other._headers), _body(other._body),
      ...
      _isParsed(other._isParsed), _isNPH(other._isNPH) {}
...
CgiResponse::ParseResult
CgiResponse::parseOutput(const std::string &cgiOutput) {
  clear();
  if (cgiOutput.empty()) {
  ...
  std::string headerSection =
      cgiOutput.substr(0, headerEndPos - separatorLength);
  _body = cgiOutput.substr(headerEndPos);
...
void CgiResponse::populateHttpResponse(HttpResponse &httpResponse) const {
  if (!_isParsed) {
    Logger::log(Logger::ERROR,
                "Cannot populate HttpResponse - CGI response not parsed");
    return;
  }
...
bool CgiResponse::isParsed() const { return _isParsed; }

bool CgiResponse::isNPH() const { return _isNPH; }
...
CgiResponse::ParseResult
CgiResponse::parseHeaders(const std::string &headerSection) {
...
CgiResponse::ParseResult
CgiResponse::parseStatusLine(const std::string &statusLine) {
