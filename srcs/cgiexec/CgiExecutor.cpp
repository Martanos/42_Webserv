...
CgiExecutor::CgiExecutor(const CgiExecutor &other) : _childPid(-1), _timeoutSeconds(other._timeoutSeconds), _processRunning(false)
{
    // Initialize pipe file descriptors to invalid state
    for (int i = 0; i < 2; ++i)
    {
...
CgiExecutor::ExecutionResult CgiExecutor::execute(const std::string &scriptPath,
...
CgiExecutor::ExecutionResult CgiExecutor::forkAndExec(const std::string &scriptPath,
                                                      const std::string &interpreter,
                                                      char **envp)
{
    _childPid = fork();
    ...
    if (_childPid == 0)
    {
        ...
        if (dup2(_stdinPipe[0].getFd(), STDIN_FILENO) == -1 ||
            dup2(_stdoutPipe[1].getFd(), STDOUT_FILENO) == -1 ||
...
CgiExecutor::ExecutionResult CgiExecutor::communicateWithChild(const std::string &inputData,
                                                               std::string &outputData,
                                                               std::string &errorData)
{
