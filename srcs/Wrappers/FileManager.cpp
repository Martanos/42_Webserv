...
FileManager::FileManager(const FileManager &src) { *this = src; }
  ...
  for (size_t attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
    try {
      ...
      if (now == static_cast<std::time_t>(-1)) {
        ...
        std::stringstream ss;
        ss << HTTP::TEMP_FILE_TEMPLATE << "pid" << getpid() << "_"
           << (++attemptCounter);
        return (ss.str());
      }
      ...
      ret = std::sprintf(buf, "%04d%02d%02d%02d%02d%02d", tm->tm_year + 1900,
      ...
      std::stringstream ss;
      ss << HTTP::TEMP_FILE_TEMPLATE << buf << "_" << getpid() << "_"
         << attempt;
      ...
      std::stringstream ss;
      ss << HTTP::TEMP_FILE_TEMPLATE << "emergency_" << getpid() << "_"
         << (++attemptCounter);
      return (ss.str());
    }
  }
...
void FileManager::instantiate() {
  ...
  _fd = FileDescriptor::createFromOpen(_filePath.c_str(),
...
void FileManager::append(const std::vector<char> &buffer,
...
std::string FileManager::getFilePath() const { return (_filePath); }

FileDescriptor &FileManager::getFd() { return (_fd); }

const FileDescriptor &FileManager::getFd() const { return (_fd); }

bool FileManager::getIsATempFile() const { return (_isATempFile); }
...
void FileManager::setFd(const FileDescriptor &fd) { _fd = fd; }
