...
HttpBody::HttpBody(HttpBody const &src) { *this = src; }
...
void HttpBody::parseBuffer(std::vector<char> &buffer, HttpResponse &response) {
  Logger::debug("HttpBody: parseBuffer called, body type: " +
                    StrUtils::toString(_bodyType) +
                    ", buffer size: " + StrUtils::toString(buffer.size()),
                __FILE__, __LINE__, __PRETTY_FUNCTION__);
  if (_bodyType == BODY_TYPE_NO_BODY) {
    Logger::debug("HttpBody: No body type, marking as complete", __FILE__,
...
HttpBody::BodyState HttpBody::_parseContentLengthBody(std::vector<char> &buffer,
  ...
  Logger::debug(
      "HttpBody: parseContentLengthBody called, expected body size: " +
          StrUtils::toString(_expectedBodySize) +
          ", is using temp file: " + StrUtils::toString(_isUsingTempFile),
      __FILE__, __LINE__, __PRETTY_FUNCTION__);
  if (!_isUsingTempFile) {
    ssize_t bytes_needed = _expectedBodySize - _rawBody.size();
    if (bytes_needed < 0) {
      Logger::debug("Body size exceeds expected size", __FILE__, __LINE__,
                    __PRETTY_FUNCTION__);
      response.setResponseDefaultBody(400, "Body size exceeds expected size",
                                      NULL, NULL, HttpResponse::FATAL_ERROR);
      return BODY_PARSING_ERROR;
    }
    ssize_t bytes_to_copy =
        std::min(bytes_needed, static_cast<ssize_t>(buffer.size()));
    _rawBody.insert(_rawBody.end(), buffer.begin(),
                    buffer.begin() + bytes_to_copy);
    ...
    if (_rawBodySize > _expectedBodySize) {
      ...
      response.setResponseDefaultBody(400, "Body size exceeds expected size",
                                      NULL, NULL, HttpResponse::FATAL_ERROR);
      return BODY_PARSING_ERROR;
    } else if (_rawBody.size() >=
               HTTP::DEFAULT_CLIENT_MAX_BODY_SIZE) // Flush to temp file
    {
...
HttpBody::BodyState HttpBody::_parseChunkedBody(std::vector<char> &buffer,
                                                HttpResponse &response) {
  while (!buffer.empty()) {
    switch (_chunkState) {
    case CHUNK_SIZE: {
      Logger::debug("HttpBody: Chunk size state", __FILE__, __LINE__,
                    __PRETTY_FUNCTION__);
      // Chunked size line validation
      std::vector<char>::iterator it =
          std::search(buffer.begin(), buffer.end(), HTTP::CRLF, HTTP::CRLF + 2);
      Logger::debug("HttpBody: Chunk size line search result: " +
                        StrUtils::toString(it - buffer.begin()),
                    __FILE__, __LINE__, __PRETTY_FUNCTION__);
      if (it == buffer.end()) // If the CRLF is not found, we need more data
      {
        if (buffer.size() >
            18) // Limit hex number size to 16 characters (8 bytes) + 2 for \r\n
        {
          Logger::debug("Chunked transfer encoding size string exceeded limit",
                        __FILE__, __LINE__, __PRETTY_FUNCTION__);
          response.setResponseDefaultBody(
              400, "Chunked transfer encoding size string exceeded limit", NULL,
      ...
      if (sizeLine.empty()) {
        Logger::debug("Empty chunk size line", __FILE__, __LINE__,
                      __PRETTY_FUNCTION__);
        response.setResponseDefaultBody(400, "Empty chunk size line", NULL,
                                        NULL, HttpResponse::FATAL_ERROR);
        return BODY_PARSING_ERROR;
      } else if (sizeLine.size() + 2 >
                 18) // 16 characters (8 bytes) + 2 for \r\n
      {
        Logger::debug("Chunked transfer encoding size string exceeded limit",
                      __FILE__, __LINE__, __PRETTY_FUNCTION__);
        response.setResponseDefaultBody(
      ...
      _rawBodySize +=
          _expectedBodySize; // Add the expected body size to the raw body size
      if (_expectedBodySize == 0) {
        Logger::debug(
            "Chunked transfer encoding size is 0, switching to trailers state",
            __FILE__, __LINE__, __PRETTY_FUNCTION__);
        _chunkState = CHUNK_TRAILERS;
        break;
      } else if (_expectedBodySize == -1) {
        _chunkState = CHUNK_ERROR;
        Logger::debug("Invalid chunk size: " + sizeLine, __FILE__, __LINE__,
                      __PRETTY_FUNCTION__);
        response.setResponseDefaultBody(400, "Invalid chunk size: " + sizeLine,
                                        NULL, NULL, HttpResponse::FATAL_ERROR);
        return BODY_PARSING_ERROR;
      }
    ...
    case CHUNK_DATA: // Use a modified form of content length body parsing
    {
      ...
      if (extractableBytes == buffer.end()) {
        if (buffer.size() >
            static_cast<size_t>(
                _expectedBodySize)) // If the buffer size is greater than the
                                    // expected body size a fatal error is
                                    // returned
        {
          Logger::debug(
              "Chunked transfer encoding body size exceeds expected size",
              __FILE__, __LINE__, __PRETTY_FUNCTION__);
          response.setResponseDefaultBody(
              400, "Chunked transfer encoding body size exceeds expected size",
              NULL, NULL, HttpResponse::FATAL_ERROR);
          return BODY_PARSING_ERROR;
        }
        return BODY_PARSING;
                         ...
                         // we could parse them
    {
      ...
      if (it == buffer.end()) {
        if (buffer.size() > HTTP::DEFAULT_CLIENT_MAX_HEADERS_SIZE) {
          Logger::debug("Chunked transfer encoding trailers line too long",
                        __FILE__, __LINE__, __PRETTY_FUNCTION__);
          response.setResponseDefaultBody(
              400, "Chunked transfer encoding trailers line too long", NULL,
              NULL, HttpResponse::FATAL_ERROR);
          return BODY_PARSING_ERROR;
        }
        return BODY_PARSING;
      } else if (it == buffer.begin()) // If its /r/n then chunks are complete
      {
        ...
        Logger::debug("Chunked transfer encoding trailers line too long",
                      __FILE__, __LINE__, __PRETTY_FUNCTION__);
        response.setResponseDefaultBody(
            400, "Chunked transfer encoding trailers line too long", NULL, NULL,
...
HttpBody::BodyState HttpBody::getBodyState() const { return _bodyState; }

HttpBody::BodyType HttpBody::getBodyType() const { return _bodyType; }
...
size_t HttpBody::getRawBodySize() const { return _rawBody.size(); }

bool HttpBody::getIsUsingTempFile() const { return _isUsingTempFile; }
...
const FileDescriptor &HttpBody::getTempFd() const { return _tempFile.getFd(); }

FileManager &HttpBody::getTempFileManager() { return _tempFile; }
/*
** --------------------------------- MUTATORS --------------------------------
*/

void HttpBody::setBodyState(BodyState bodyState) { _bodyState = bodyState; }

void HttpBody::setBodyType(BodyType bodyType) { _bodyType = bodyType; }
