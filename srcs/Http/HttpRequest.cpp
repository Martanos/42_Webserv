...
bool HttpRequest::_identifyServer(HttpResponse &response) {
  if (_potentialServers == NULL) {
    Logger::error("HttpRequest: No potential servers found", __FILE__, __LINE__,
                  __PRETTY_FUNCTION__);
    response.setResponseDefaultBody(500, "Internal Server Error", NULL,
                                    HttpResponse::FATAL_ERROR);
    return false;
  }
  const Header *hostHeader = _headers.getHeader("host");
  if (hostHeader == NULL) {
    Logger::error("HttpRequest: No host header found", __FILE__, __LINE__,
                  __PRETTY_FUNCTION__);
    response.setResponseDefaultBody(400, "No host header found", NULL,
                                    HttpResponse::FATAL_ERROR);
    return false;
  }
  std::string hostValue = (!hostHeader->getValues().empty())
                              ? hostHeader->getValues()[0]
                              : "<none>";
  ...
  if (!hostPart.empty() && hostPart[0] == '[') {
    ...
    if (rb != std::string::npos) {
      std::string after =
          (rb + 1 < hostPart.size()) ? hostPart.substr(rb + 1) : std::string();
    ...
    if (first != std::string::npos && first == last) {
      ...
      portPart = hostPart.size() < hostValue.size()
                     ? hostValue.substr(first + 1)
                     : std::string();
    }
  }
  ...
  for (std::vector<Server>::const_iterator it = servers.begin();
    ...
    if (it->hasServerName(_selectedServerHost)) {
      if (!_selectedServerPort.empty()) {
        for (std::vector<SocketAddress>::const_iterator it2 =
                 it->getSocketAddresses().begin();
  ...
  response.setResponseDefaultBody(404,
                                  "Matching server configuration not found",
                                  NULL, HttpResponse::FATAL_ERROR);
  Logger::debug(
      "HttpRequest: Matching server configuration not found for host: " +
          hostValue,
      __FILE__, __LINE__, __PRETTY_FUNCTION__);
  return false;
}
...
bool HttpRequest::_identifyLocation(HttpResponse &response) {
  try {
    ...
    Logger::debug("HttpRequest: Matched location: " +
                      _selectedLocation->getLocationPath() +
                      " for URI: " + _uri.getURI(),
                  __FILE__, __LINE__, __PRETTY_FUNCTION__);
  } catch (const std::exception &e) {
    Logger::error("HttpRequest: Exception during location lookup: " +
                      std::string(e.what()) + " for URI: " + _uri.getURI(),
                  __FILE__, __LINE__, __PRETTY_FUNCTION__);
    response.setResponseDefaultBody(
        500, "Exception during location lookup: " + std::string(e.what()), NULL,
        HttpResponse::FATAL_ERROR);
    return false;
  }
  ...
  {
    response.setResponseDefaultBody(
        404, "No matching location found for URI: " + _uri.getURI(), NULL,
        HttpResponse::ERROR);
    return false;
  }
  // 3. Once location is found sanitize the request (can only be done after
  // location is found)
...
HttpRequest::ParseState
HttpRequest::parseBuffer(std::vector<char> &holdingBuffer,
                         HttpResponse &response) {
  PERF_SCOPED_TIMER(http_request_parsing);

  Logger::debug("HttpRequest: parseBuffer called, state: " +
                    StrUtils::toString(_parseState) + ", buffer size: " +
                    StrUtils::toString(holdingBuffer.size()),
                __FILE__, __LINE__, __PRETTY_FUNCTION__);
  ...
  while (_parseState != PARSING_COMPLETE && _parseState != PARSING_ERROR &&
    ...
    switch (_parseState) {
    case PARSING_URI: {
      Logger::debug("HttpRequest: Parsing URI", __FILE__, __LINE__,
                    __PRETTY_FUNCTION__);
      _uri.parseBuffer(holdingBuffer, response);
      Logger::debug("HttpRequest: URI state: " +
                        StrUtils::toString(_uri.getURIState()),
                    __FILE__, __LINE__, __PRETTY_FUNCTION__);
      switch (_uri.getURIState()) {
      case HttpURI::URI_PARSING_COMPLETE: {
        Logger::debug("HttpRequest: URI parsing complete", __FILE__, __LINE__,
                      __PRETTY_FUNCTION__);
        Logger::debug("URI: " + _uri.getMethod() + " " + _uri.getURI() + " " +
                          _uri.getVersion(),
                      __FILE__, __LINE__, __PRETTY_FUNCTION__);
        _parseState = PARSING_HEADERS;
        Logger::debug("HttpRequest: Transitioned to PARSING_HEADERS", __FILE__,
                      __LINE__, __PRETTY_FUNCTION__);
        break;
      }
      case HttpURI::URI_PARSING_ERROR:
        Logger::error("HttpRequest: Request line parsing error", __FILE__,
                      __LINE__, __PRETTY_FUNCTION__);
    ...
    case PARSING_HEADERS: {
      Logger::debug("HttpRequest: Starting headers parsing", __FILE__, __LINE__,
                    __PRETTY_FUNCTION__);
      _headers.parseBuffer(holdingBuffer, response, _body);
      Logger::debug("HttpRequest: Headers parsing state: " +
                        StrUtils::toString(_headers.getHeadersState()),
                    __FILE__, __LINE__, __PRETTY_FUNCTION__);
      ...
      case HttpHeaders::HEADERS_PARSING_COMPLETE: {
        ...
        Logger::debug("HttpRequest: Headers parsing complete", __FILE__,
                      __LINE__, __PRETTY_FUNCTION__);
        ...
        case HttpBody::BODY_TYPE_CHUNKED:
          ...
          break;
        default:
          Logger::error("HttpRequest: Invalid body type", __FILE__, __LINE__,
                        __PRETTY_FUNCTION__);
          _parseState = PARSING_ERROR;
          break;
        }
    ...
    case PARSING_BODY: {
      Logger::debug("HttpRequest: Parsing body", __FILE__, __LINE__,
                    __PRETTY_FUNCTION__);
      _body.parseBuffer(holdingBuffer, response);
      Logger::debug("HttpRequest: Body state: " +
                        StrUtils::toString(_body.getBodyState()),
                    __FILE__, __LINE__, __PRETTY_FUNCTION__);
      switch (_body.getBodyState()) {
      case HttpBody::BODY_PARSING_COMPLETE: {
        Logger::debug("HttpRequest: Body parsing complete", __FILE__, __LINE__,
                      __PRETTY_FUNCTION__);
        _parseState = PARSING_COMPLETE;
        Logger::debug("HttpRequest: Transitioned to PARSING_COMPLETE", __FILE__,
                      __LINE__, __PRETTY_FUNCTION__);
        break;
      }
  ...
  Logger::debug("HttpRequest: parseBuffer returning state: " +
                    StrUtils::toString(_parseState),
                __FILE__, __LINE__, __PRETTY_FUNCTION__);
  return _parseState;
}
...
void HttpRequest::setPotentialServers(
    const std::vector<Server> *potentialServers) {
...
void HttpRequest::setTempFile(const std::string &FilePath) {
  _body.getTempFileManager().setFilePath(FilePath);
}
...
HttpRequest::getQueryParameters() const {
...
// URI accessors
std::string HttpRequest::getMethod() const { return _uri.getMethod(); };

std::string HttpRequest::getUri() const { return _uri.getURI(); };

std::string HttpRequest::getRawUri() const { return _uri.getRawURI(); };

std::string HttpRequest::getVersion() const { return _uri.getVersion(); };
...
std::string HttpRequest::getBodyData() const { return _body.getRawBody(); };
...
std::string HttpRequest::getSanitizedUri() const {
  return _uri.getSanitizedURI();
}
...
void HttpRequest::incrementInternalRedirectDepth() { _internalRedirectDepth++; }

void HttpRequest::resetInternalRedirectDepth() { _internalRedirectDepth = 0; }
