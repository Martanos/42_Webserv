...
//	_config = serverConfig;
	_config = (serverConfig ? new ServerConfig(*serverConfig) : NULL);  // Deep copy: owns lifetime
	_locations = std::map<std::string, Location>();
}
...
Server &Server::operator=(Server const &rhs)
{
	if (this != &rhs)
	{
		...
		_port = rhs._port;
//		_config = rhs._config;
		_config = (rhs._config ? new ServerConfig(*(rhs._config)) : NULL);  // Deep copy
