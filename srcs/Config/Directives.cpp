...
Directives::Directives()
    : _rootPath(""), _autoIndexValue(false), _isCgiPathValue(false),
      _uploadPath(""), _keepAliveValue(false), _clientMaxBodySize(-1.0),
      _redirect(std::pair<int, std::string>()), _hasRootPathDirective(false),
      _hasAutoIndexDirective(false), _hasisCgiPathDirective(false),
      _hasUploadPathDirective(false), _hasKeepAliveDirective(false),
      _hasClientMaxBodySizeDirective(false), _hasRedirectDirective(false),
      _hasIndexDirective(false), _hasStatusPathDirective(false),
...
bool Directives::hasIndexDirective() const { return (_hasIndexDirective); }
...
const std::string *Directives::getRootPath() const { return (&_rootPath); }
bool Directives::getAutoIndexValue() const { return (_autoIndexValue); }
bool Directives::getisCgiPathValue() const { return (_isCgiPathValue); }
const std::string *Directives::getUploadPath() const { return (&_uploadPath); }
bool Directives::getKeepAliveValue() const { return (_keepAliveValue); }
double Directives::getClientMaxBodySize() const { return (_clientMaxBodySize); }
...
const std::string *Directives::getStatusPath(int status) const {
  for (std::map<int, std::string>::const_iterator it = _statusPaths.begin();
       it != _statusPaths.end(); ++it) {
    if (it->first == status)
      return (&(it->second));
  }
  return (NULL);
...
void Directives::insertStatusPath(const std::vector<int> &codes,
  ...
  for (std::vector<int>::const_iterator it = codes.begin(); it != codes.end();
       ++it) {
...
void Directives::setAllowedMethods(
