...
PerformanceMonitor::Timer::Timer(const std::string &operationName) : _isRunning(false), _operationName(operationName)
{
	_startTime.tv_sec = 0;
...
double PerformanceMonitor::Timer::getElapsedTime() const
{
	if (!_isRunning)
	{
		return ((_endTime.tv_sec - _startTime.tv_sec) * 1000.0) + ((_endTime.tv_usec - _startTime.tv_usec) / 1000.0);
	}
	else
	{
...
void PerformanceMonitor::recordConnection()
{
	...
	Logger::debug("PerformanceMonitor: Connection recorded. Active: " +
					  StrUtils::toString(_currentMetrics.activeConnections),
...
std::string PerformanceMonitor::generatePerformanceSummary() const
{
	...
	ss << "  Avg Response Time: " << std::fixed << std::setprecision(2) << _sessionMetrics.averageRequestTime
	   << " ms\n";
	ss << "  Success Rate: " << std::fixed << std::setprecision(1)
	   << (_sessionMetrics.totalRequests > 0
			   ? (100.0 * _sessionMetrics.successfulRequests / _sessionMetrics.totalRequests)
			   : 0.0)
	   << "%\n";
	ss << "  Active Connections: " << _sessionMetrics.activeConnections << "\n";
...
void PerformanceMonitor::logSystemStatus()
{
	...
	Logger::info("  Memory Usage: " + StrUtils::toString(_currentMetrics.currentMemoryUsage / 1024) + " KB", __FILE__,
				 __LINE__, __FUNCTION__);
	Logger::info("  Peak Memory: " + StrUtils::toString(_currentMetrics.peakMemoryUsage / 1024) + " KB", __FILE__,
				 __LINE__, __FUNCTION__);
	Logger::info("  File Descriptors: " + StrUtils::toString(_currentMetrics.fileDescriptorsUsed) + "/" +
					 StrUtils::toString(_currentMetrics.maxFileDescriptors),
				 __FILE__, __LINE__, __FUNCTION__);
	Logger::info("  Active Connections: " + StrUtils::toString(_currentMetrics.activeConnections), __FILE__, __LINE__,
				 __FUNCTION__);
}
...
void PerformanceMonitor::setPerformanceThresholds(double maxRequestTime, double maxCGITime, size_t maxMemoryUsage)
{
	...
	Logger::info("PerformanceMonitor: Thresholds set - Max Request Time: " + StrUtils::toString(maxRequestTime) +
					 " ms, Max CGI Time: " + StrUtils::toString(maxCGITime) +
					 " ms, Max Memory: " + StrUtils::toString(maxMemoryUsage / 1024) + " KB",
				 __FILE__, __LINE__, __FUNCTION__);
}
