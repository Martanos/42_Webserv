...
void Client::_routeRequest()
{
	...
	Logger::debug("Client: Routing request for " + uri.getResolvedPath() +
					  ", location type: " + StrUtils::toString(location->getLocationType()) +
	...
	if (std::find(location->getAllowedMethods()->begin(), location->getAllowedMethods()->end(),
				  uri.getMethod()) == location->getAllowedMethods()->end()) // 2. Verify method is allowed
	{
	...
	case Location::UPLOAD:
	{
		...
		{
			Logger::error("Client: Failed to create handler for method: " + uri.getMethod(), __FILE__, __LINE__,
						  __PRETTY_FUNCTION__);
			_response.setResponseDefaultBody(403, "Method Not Implemented: " + uri.getMethod(), location,
											 HttpResponse::ERROR);
		}
		break;
	}
	default:
		Logger::error("Client: Unknown location type for routing request", __FILE__, __LINE__, __PRETTY_FUNCTION__);
		_response.setResponseDefaultBody(500, "Unknown location type for routing request", location,
										 HttpResponse::ERROR);
		break;
	}
}
...
void Client::_handleResponseBuffer()
{
	...
	while (totalBytesSent < HTTP::DEFAULT_SEND_SIZE && !_responseBuffer.empty())
	{
		...
		case HttpResponse::RESPONSE_SENDING_ERROR: // Fatal error encountered sending the response
