...
void Client::_routeRequest() {
  ...
  IMethodHandler *handler =
      MethodHandlerFactory::createHandler(_request.getMethod());
  if (handler) {
    ...
    case Location::UPLOAD:
      handler->handleRequest(_request, _response, _request.getSelectedServer(),
                             location);
      break;
    case Location::CGI:
      handler->handleCgiRequest(_request, _response,
                                _request.getSelectedServer(), location);
      break;
    case Location::REDIRECT:
      _response.setRedirectResponse(location->getRedirect()->first,
                                    location->getRedirect()->second,
    ...
    _response.setResponseDefaultBody(
        403, "Method Not Implemented: " + _request.getMethod(), location,
        HttpResponse::ERROR);
  }
}
